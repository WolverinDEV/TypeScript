...
namespace ts {
    ...
    function explainFiles(program: ProgramToEmitFilesAndReportErrors, write: (s: string) => void) {
        ...
        for (let order = FileIncludeKind.RootFile; order <= FileIncludeKind.AutomaticTypeDirectiveFile; order++) {
            ...
            for (const file of files) {
                write(`${file.fileName}${file.redirectInfo ? " -> " + file.redirectInfo.redirectTarget.fileName : ""}`);
                for (const reason of reasons.get(file.path)!) {
                    if (reason.kind !== order || isReferencedFileKind(order)) {
    ...
    function explainFileIncludeReason(program: ProgramToEmitFilesAndReportErrors, reason: FileIncludeReason) {
        if (isReferencedFile(reason)) {
            const { file, pos, end } = getReferencedFileLocation(path => program.getSourceFileByPath(path), reason);
            return `  ${FileIncludeKind[reason.kind]}:: ${file.text.substring(pos, end)} from ${file.fileName} ${reason.index}`;
        }
        if (reason.kind === FileIncludeKind.ProjectReferenceFile) {
            return `  ${FileIncludeKind[reason.kind]}:: from ${reason.config}`;
        }
